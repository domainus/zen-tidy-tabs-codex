...
(() => {
  ...
  function tabUrlForPrompt(tab) {
    try {
      const spec = (tab.linkedBrowser || gBrowser?.getBrowserForTab?.(tab))?.currentURI?.spec || "";
      const parsed = new URL(spec);
      parsed.hash = "";
      parsed.search = "";
      return tidyPref(CONFIG.PREF_SEND_PATHS, false) ? parsed.toString() : parsed.hostname;
    } catch {
      return "";
    }
  }

  function codexPromptPayload(tabs) {
    return tabs.map((tab, index) => ({
      index,
      title: getTabTitle(tab),
      url: tabUrlForPrompt(tab),
    }));
  }

  function parseTopicJson(text, tabs) {
